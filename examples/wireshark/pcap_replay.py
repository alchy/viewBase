...
def make_resolver(canvas: vb.Canvas):
    ...
    def _resolve(ip: str) -> None:
        ...
        canvas.update_node(ip, fqdn=fqdn)   # label "{fqdn} [{ip}]" se sestaví sám
...
def build_canvas() -> vb.Canvas:
    ...
    canvas.node_label("{fqdn} [{ip}]")   # title se sestaví z meta uzlu
        ...
        with canvas.batch():
            for node_id in (src, dst):
                if node_id not in nodes:
                    nodes.add(node_id)
                    canvas.add_node(node_id, type="host", ip=node_id, fqdn="")
